...
# Convertir la columna 'Hora' a formato de tiempo si no está en el formato adecuado
combined_df['Hora'] = pd.to_datetime(combined_df['Hora'], format='%H:%M', errors='coerce').dt.time

# Verificar el cambio
print(combined_df['Hora'].head())

# 1. Gráfico de Series Temporales para Precios por Hora
plt.figure(figsize=(12, 6))
plt.plot(combined_df['Hora'], combined_df['Precio Compra/Venta'], marker='o', linestyle='-')
...
# 2. Gráfico de Cajas para Precios
plt.figure(figsize=(10, 6))
sns.boxplot(data=combined_df, x='Precio Compra/Venta')
plt.title('Distribución de Precios Compra/Venta')
plt.xlabel('Precio Compra/Venta')
plt.grid(True)
plt.tight_layout()
plt.show()

# 3. Gráfico de Barras para Energía Comprada y Vendida
# Convertir la columna 'Energia Compra/Venta' a float para poder sumar
combined_df['Energia Compra/Venta'] = combined_df['Energia Compra/Venta'].str.replace('.', '').str.replace(',', '.').astype(float)

# Agrupar por 'Fecha' y sumar las cantidades de energía comprada y vendida
energy_summary = combined_df.groupby('Fecha')['Energia Compra/Venta'].sum().reset_index()

plt.figure(figsize=(12, 6))
sns.barplot(data=energy_summary, x='Fecha', y='Energia Compra/Venta')
plt.title('Energía Comprada y Vendida por Fecha')
plt.xlabel('Fecha')
plt.ylabel('Energia Compra/Venta')
plt.xticks(rotation=45)  # Rotar las etiquetas del eje x para mejor legibilidad
plt.grid(True)
plt.tight_layout()
plt.show()
